(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.AlertingWebApi.ApiClient = factory(root.superagent);
  }
  ...
  var ApiClient = function ApiClient() {
    ...
    this.authentications = {
      'headerToken': {type: 'apiKey', in: 'header', name: 'X-Auth-Token'}
    };
  ...
  ApiClient.prototype.appKey = function appKey(apiKey) {
    if (apiKey) {
      for (var key in this.authentications) {
        var val = this.authentications[key];
        if (val && val.type && val.type === 'apiKey') {
            val.apiKey = apiKey;
        }
      }
    }
